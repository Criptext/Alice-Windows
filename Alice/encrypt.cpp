...
int postEncryptEmailGroup(struct mg_connection *conn, void *cbdata, char *dbPath, char *password) {
  ...
  spdlog::info("[{0}] /encrypt/email/group Receiving Request", endpointId);
  ...
  cJSON *obj = cJSON_Parse(reinterpret_cast<char *>(decryptedResponse->data));
  cJSON *accountRecipientId, *deviceIds, *recipientId, *body, *preview, *fileKeys;
  accountRecipientId = cJSON_GetObjectItemCaseSensitive(obj, "accountRecipientId");
  deviceIds = cJSON_GetObjectItemCaseSensitive(obj, "deviceIds");
  recipientId = cJSON_GetObjectItemCaseSensitive(obj, "recipientId");
  body = cJSON_GetObjectItemCaseSensitive(obj, "body");
  preview = cJSON_GetObjectItemCaseSensitive(obj, "preview");
  fileKeys = cJSON_GetObjectItemCaseSensitive(obj, "fileKeys");

  if (!cJSON_IsString(accountRecipientId) || !cJSON_IsString(recipientId) || !cJSON_IsArray(deviceIds)) {
    sendError(conn, 400, "Missing Params");
    return 400;
  }
  ...
  cJSON *deviceId = NULL;
  cJSON_ArrayForEach(deviceId, deviceIds) {
    cJSON *recipientDeviceResponse = cJSON_CreateObject();
    ...
    if (cJSON_IsString(body)) {
      try {
        ...
        cJSON_AddStringToObject(recipientDeviceResponse, "bodyEncrypted", encryptedBody);
        cJSON_AddNumberToObject(recipientDeviceResponse, "bodyMessageType", type);
      } catch (exception &ex) {
        (void)[v = std::move(db)]{};
        spdlog::error("[{0}] ENCRYPT BODY ERROR {1}", endpointId, ex.what());
        sendError(conn, 500, ex.what());
    ...
    if (cJSON_IsString(preview)) {
      try {
        ...
        cJSON_AddStringToObject(recipientDeviceResponse, "previewEncrypted", encryptedPreview);
        cJSON_AddNumberToObject(recipientDeviceResponse, "previewMessageType", type);
      } catch (exception &ex) {
        (void)[v = std::move(db)]{};
        spdlog::error("[{0}] ENCRYPT PREVIEW ERROR {1}", endpointId, ex.what());
        sendError(conn, 500, ex.what());
    ...
    if (cJSON_IsArray(fileKeys)) {
      ...
      cJSON_AddItemToObject(recipientDeviceResponse, "fileKeysEncrypted", myFileKeys);
    }

    cJSON_AddItemToObject(response, to_string(deviceId->valueint).c_str(), recipientDeviceResponse);
  }
